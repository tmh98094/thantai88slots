...
test("makeOutputPath uses the safe docs seo-ops folders", () => {
  ...
  assert.equal(
    makeOutputPath("drafts", "slots-brief", new Date("2026-07-05T00:00:00Z")),
    "docs/seo-ops/drafts/2026-07-05-slots-brief.md",
  );
});
...
test("buildReportMarkdown includes human review and no auto-publish guardrails", () => {
  const report = buildReportMarkdown({
    ...
    missingEnv: [{ env: "PAGESPEED_API_KEY", label: "PageSpeed Insights" }],
  ...
  assert.match(report, /PAGESPEED_API_KEY/);
